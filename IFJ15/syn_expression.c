...
int deleteSTStack(sTreeStack *STST){
  sTreeStack pomSTST=(*STST);
  free(pomSTST);
  return true;
}

int deleteSynTree(sTree SyntTree){
  if(SyntTree==NULL) return true;
  else{
    deleteSynTree(SyntTree->LSynTree);
    deleteSynTree(SyntTree->RSynTree);
    free(SyntTree);
...
sTree syn_exp(int inittype,string *inits,FN newFN,constTable newCTable,int endType){
  ...
  sTree act;
  // zpracovani prvniho lexemu z vyrazu
  if(inittype==ID){
  ...
  while(1){
    c=comp(STST,act);
    if(c==STLESS){//<     Vlozeni zarazky a lexemu z lexikalni analyzy na vrchol zasobniku
    ...
    else if(c==STEQ){ //=  Vlozeni lexemu z lexikalni analyzy na vrchol zasobniku
      pushSTStack(&STST,act);
      string str;
      strInit(&str);
      int type=getNextToken(&str);
    ...
    else if(c==STGREAT){//>     vyber a zpracovani dat od vrcholu zasobniku az po prvni zarazku
    ...
    else if(c==STEND){// vyraz byl syntakticky spravne, vraci se vrchol syntaktickeho stromu
      sTree pomsTree=STST->First;
      deleteSTStack(&STST);
      return pomsTree;
    }
    else { // vyraz byl syntakticky spatne, vraci se NULL
        deleteSynTree(STST->First);
        deleteSTStack(&STST);
...
int comp(sTreeStack STST,sTree ST0){
  ...
  int typeSTpom=convConstTypes(STpom->stype);
